...
if 1 " custom key mapping
    ...
    if !g:zf_fakevim
        ...
        nnoremap <silent> Y :call ZF_Setting_join('')<cr>
        xnoremap <silent> Y :call ZF_Setting_join('')<cr>
...
if 1 " common settings
    ...
    augroup END
    function! ZF_Setting_isLargeFile()
        if get(b:, 'zf_vim_largefile_force', 0)
            return 1
        endif
        let largeLine = get(g:, 'ZF_Setting_largefile_line', 5000)
        if largeLine > 0 && line('$') >= largeLine
            return 1
        endif
        ...
        if filereadable(file)
            ...
            if largeColumn > 0
                for line in readfile(file, '', get(g:, 'ZF_Setting_largefile_columnChecklines', 5))
            ...
            if largeColumn > 0
                let nLine = get(g:, 'ZF_Setting_largefile_columnChecklines', 5)
                if nLine > line('$')
                    let nLine = line('$')
                endif
                for i in range(1, nLine)
                    if len(getline(i)) >= largeColumn
                        return 1
                    endif
                endfor
            endif
    ...
    if !g:zf_fakevim
        augroup ZF_Setting_largefile_augroup
            ...
            function! s:ZF_Setting_largefile_setup(notifyRestore)
                if !get(b:, 'zf_vim_largefile', 0) && ZF_Setting_isLargeFile()
                    set eventignore+=FileType
                    if has('timers')
            ...
            autocmd TextChanged * call s:ZF_Setting_largefile_setup(0)
        augroup END
    endif
    " encodings
